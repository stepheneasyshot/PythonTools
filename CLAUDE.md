...
7. **更新 README.md**: 在项目结构、工具详情表、示例代码中同步补充说明
8. **更新 CLAUDE.md**: 同步更新模块计数

## 自动化规则（Harness 协议）

以下规则必须严格遵守，每次执行相关操作时自动触发：

### 新增功能 → 同步更新 README.md
- **触发条件**: 任何时候新增工具模块、函数、菜单项或项目结构变更
- **必须操作**: 同步更新 README.md 的以下三个部分：
  1. 项目结构树（模块计数 + 新增文件）
  2. 工具详情表格（新函数、参数、说明）
  3. 示例代码（展示典型用法）
- **Why**: README.md 是用户的第一入口，与代码不一致会导致使用混乱
- **How to apply**: 在完成代码变更后，立即检查 README.md 是否需要同步；不要等到最后一次性更新
